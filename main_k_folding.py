...
training_dataset = ImageDataset(TRAINING_DATA_DIR, CLASSES, transform_train)
val_dataset = ImageDataset(VALIDATION_DATA_DIR, CLASSES, transform_val)
full_dataset = ConcatDataset([training_dataset, val_dataset])

kf      = KFold(n_splits=5, shuffle=True, random_state=42)
indices = np.arange(len(full_dataset))

print(f"Total imágenes para K-Fold: {len(full_dataset)}")
